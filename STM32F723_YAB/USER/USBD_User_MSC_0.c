...
extern   uint32_t RamDrive[192*1024/4] __attribute__((section("RAM_Drive")));   // Memory in RAM for dummy disk image

//__IO uint8_t memory[192*1024/4] __attribute__((at(0x20001000)));

//uint32_t memory[192*1024/4] __attribute__((section(".ARM.__at_0x20001000"))); //set RAM DISK start address

extern void MSCformat(void);
//extern const uint8_t memory_disk_image[];  // Dummy Memory Disk Image
#endif
...
#else
  *block_count = sizeof(RamDrive)/512U;   // Total number of blocks on media
...
bool USBD_MSC0_Read (uint32_t lba, uint32_t cnt, uint8_t *buf) {
  ...
  memcpy (buf, &RamDrive[lba * (512U/4U)], cnt * 512U);
#endif
  return true;
}
...
bool USBD_MSC0_Write (uint32_t lba, uint32_t cnt, const uint8_t *buf) {
  ...
  memcpy (&RamDrive[lba * (512U/4U)], buf, cnt * 512U);
  osThreadFlagsSet(BOOTLOADER_ThreadId, 0x01U);
#endif
  return true;
}
